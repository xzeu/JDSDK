...
SYSTEM_GENERATE_VERSION = "jing-sdk-python-20190726"
P_APPKEY = "app_key"
P_API = "method"
P_SESSION = "session"
...
class RestApi(object):
    ...
    def __init__(self, domain='gw.api.taobao.com', port=80):
        ...
        if (jing.getDefaultAppInfo()):
            self.__app_key = jing.getDefaultAppInfo().appkey
            self.__secret = jing.getDefaultAppInfo().secret
    ...
    def set_app_info(self, appinfo):
        # =======================================================================
        # 设置请求的app信息
        # @param appinfo: import jing
        #                 appinfo jing.appinfo(appkey,secret)
        # =======================================================================
    ...
    def getResponse(self, authrize=None, timeout=30):
        ...
        url = self.__domain + "?" + urllib.parse.urlencode(sys_parameters)

        response = requests.request(self.__httpmethod, url, params=body, headers=header)

        if response.status_code is not 200:
            raise RequestException(
                'invalid http status ' + str(response.status_code) + ',detail body:' + response.content)
        ...
        if "error_response" in jsonobj:
            ...
            if P_CODE in jsonobj["error_response"]:
                error.code = jsonobj["error_response"][P_CODE]
            # if jsonobj["error_response"].has_key(P_MSG) :
            if P_ZDESC in jsonobj["error_response"]:
                error.zh_desc = jsonobj["error_response"][P_ZDESC]
